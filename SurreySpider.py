...
class SurreySpider(scrapy.Spider):
    ...
    def parse_course(self, response):
        try:
            ...
            #5 degree level
            degree_level = response.xpath('//span[@class="pb-2 border-top-0 border-right-0 border-left-0 border border-dark"]').extract()
            ...
            #course_structure
            course_structure = response.xpath('//div[@id="modules-0"]').extract()
    ...
    def parse(self,response):
        url=" "
        TOFEL = self._get_TOFEL_score(url)
        PTE = self._get_PTE_score(url)
        yield scrapy.Request(url,callback=self.parse)

    def __get__TOFEL_score(self,url):
